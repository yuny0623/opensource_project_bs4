...
from collect_text import select_text
from create_obj import create_some
from create_list_soup import make_small_list
class url_parse:
    '''
    ...
    def set_word(self,out_word): #찾을 단어를 지정한다. 
        url_parse.word_to_find = out_word 

    #count_word는 정상 작동함. 직접확인해보면 암. 
    def count_word(self,out_word_list): #해당 단어의 노출 빈도를 반환한다. 
        for i in range(len(out_word_list)):
            if url_parse.word_to_find in out_word_list[i]:
                url_parse.word_count+=1
        return url_parse.word_count
    ...
    def show_page_include_word(self,y_list):
        ...
        for i in range(len(url_parse.soup_list)):
            for j in range(len(url_parse.soup_list[i])):
                # if int(url_parse.soup_list[i][j].find(url_parse.word_to_find)) != (-1):
                #     store_url_location.append(y_list[i*10+j])
                if url_parse.word_to_find in url_parse.soup_list[i][j]:
                    store_url_location.append(y_list[i*10+j])
