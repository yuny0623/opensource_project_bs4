...
from make_small_list import make_small_list
class url_to_tree:
    '''
    ...
    def set_word(self,out_word): #찾을 단어를 지정한다. 
        url_to_tree.word_to_find = out_word 

    #count_word는 정상 작동함. 직접확인해보면 암. 
    def count_word(self,out_word_list): #해당 단어의 노출 빈도를 반환한다. 
        for i in range(len(out_word_list)):
            if url_to_tree.word_to_find in out_word_list[i]:
                url_to_tree.word_count+=1
        return url_to_tree.word_count
    ...
    def search_deeper(self,out_url_list): #일단은 100개 이하인것만 처리 가능.불필요한 반복이 너무 많음. 
        ...
        if (len(li)<400) & (len(li) >300): # 400>x>300
            ...
            for i in range(int(len(li)/10)): #int로 나누기연산 감싸야됨. 안하면 float 적용됨. 에러 
                url_to_tree.soup_list.append(make_small_list().list_seperate(li[j:(j+10)]))
                j+=10
                time.sleep(1)
                if i == (int(len(li)/10)):
                    url_to_tree.soup_list.append(make_small_list().list_seperate(li[j:]))
        ...
        if (len(li)<10) & (len(li)>0): #10>x>0
            url_to_tree.soup_list.append(make_small_list().list_seperate(li))

        if len(li) <0:
            raise Exception("잘못된 값이 입력되었습니다.")
        return url_to_tree.soup_list
    

    #반드시 search_deeper 호출 다음에 호출되어야함. 아니면 NULL값 리던 .
    def show_page_include_word(self,y_list):
        store_url_location = [] #단어가 존재하는 url의 위치를 저장한다. 
        if len(y_list) == 0:
            raise Exception("search_deeper가 호출되지 않았습니다. ")
        for i in range(len(url_to_tree.soup_list)):
            for j in range(len(url_to_tree.soup_list[i])):
                # if int(url_to_tree.soup_list[i][j].find(url_to_tree.word_to_find)) != (-1):
                #     store_url_location.append(y_list[i*10+j])
                if url_to_tree.word_to_find in url_to_tree.soup_list[i][j]:
                    store_url_location.append(y_list[i*10+j])
